...
console.log("asdfasdf",senderId,receiverId)
  try {
    // Find messages where sender/receiver match either pair of users
    const messages = await Chat.find({
      $or: [
        { sender: senderId, receiver: receiverId },
        { sender: receiverId, receiver: senderId },
      ],
    }).sort({ createdAt: 1 }); // Sort by creation time (optional)
...
exports.getUsersWithAdminMessages = async (req, res) => {
  ...
  try {
    ...
    const filteredUserIds = userIds.filter(
      (userId) => userId.toString() !== adminId
    );
